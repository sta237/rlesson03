...
$$Y_i\sim\text{Ber}\left(\frac{1}{4}\right)\quad\text{for }i=1,2,3,\dots,10$$
$$X=\sum_{i=1}^{10}Y_i\sim\text{Bin}\left(10,\frac{1}{4}\right)$$

For a binomial distribution, we can use **`dbinom`**, **`pbinom`**, **`rbinom`**, and **`qbinom`**.

To compute $p_X(x)$ for $x=0,1,2,3,\dots,10$, we use **`dbinom`**.

```{r echo=TRUE}
pmfX <- dbinom(0:10, 10, 1/4)
# creating a data frame to represent data
binomX <- data.frame(x = 0:10, pmf = pmfX) 
binomX
```
...
ggplot(binomX, aes(x = x)) + # initialize a ggplot canvas with the data
...
library(patchwork)
library(tidyverse)
ppmf <- ggplot(binomX, aes(x = x)) + 
  theme_classic() + 
  geom_bar(aes(y = pmf), stat = 'identity', width =  0.02) +
  geom_point(aes(y = pmf)) +
  labs(y = expression(p[X]), title = "Probability mass function") +
  ylim(0,1)
pcdf <- ggplot(binomX, aes(x = x, y = cdf)) + 
  theme_classic() +
  geom_segment(aes(xend = lead(x), yend = cdf), stat = 'identity', width =  0.02) +
  geom_point() +
  labs(y = expression(F[X]), title = "Cumulative distribution function") +
  ylim(0,1)
# plot both
ppmf +  pcdf
```
